...
// @Success 200 {string} models.Object.resource
// @Failure 403 username or password is empty
// @router /getTreeGrid [post]
func (this *ResourceController) GetTreeGrid() {
	tree := models.ResourceTreeGrid()

	//转换UrlFor 2 LinkUrl
	this.UrlFor2LinkNew(tree)
...
//UrlFor2Link 使用URLFor方法，批量将资源表里的UrlFor值转成LinkUrl(konger修改)
func (this *ResourceController) UrlFor2LinkNew(src []*models.Resource) {
	for _, item := range src {
		item.LinkUrl = this.UrlFor2LinkOne(item.UrlFor)
		this.UrlFor2LinkNewSons(item.Sons)
	}
}
func (this *ResourceController) UrlFor2LinkNewSons(src []*models.Resource) {
	for _, item := range src {
		item.LinkUrl = this.UrlFor2LinkOne(item.UrlFor)
		this.UrlFor2LinkNewSons(item.Sons)
	}
}
//将父节点数组转换为前端选择框数组（konger修改）
func(this *ResourceController)GridToArray(src []*models.Resource)[]*models.Resource{

	result := make([]*models.Resource, 0)
	for _, item := range src {
		if item.Parent == nil || item.Parent.Id == 0 {
			result = append(result, item)
			result = this.GridToArraySon(item, result)
		}
	}
	return result
}
//将父节点数组转换为前端选择框数组（konger修改）
func(this *ResourceController)GridToArraySon(cur *models.Resource,result []*models.Resource) []*models.Resource{
	for _, item := range cur.Sons {
			if item.Level<3{
				blank :=""
				//根据级别增加菜单前面的空白数量
